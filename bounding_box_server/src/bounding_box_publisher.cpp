 ...
 */
void BoundingBoxPublisher::publishBoundingBoxes(std::vector<bounding_box_server::BoundingBox> bounding_boxes) {
  ...
  for (auto bounding_box: bounding_boxes) {
    visualization_msgs::Marker marker;
		marker.header.frame_id = "base_link"; // Transform from the "/root" frame of reference (used to transform the Point Cloud to)
		marker.header.stamp = ros::Time::now();
		marker.type = visualization_msgs::Marker::LINE_LIST;
		marker.lifetime = ros::Duration(2.0); // Keep the lines a life (visible) for 2 seconds 
		marker.id = marker_id;
