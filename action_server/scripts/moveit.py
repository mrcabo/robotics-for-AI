...
class MoveIt(object):
    ...
    def tiago_move_to_nav_position(self):
        ...
        joint_position = [1.21815663190754, -0.3101285880528266, -1.666504666674431, 1.8833762301963013,
                          -1.8310526753354068, 1.5179019342751836, 1.7070040761740843]
        result = self.move_joints(joint_position)

        if not result:
            return False

        joint_position = [0.560437389213778, -1.2417208647320237, -0.8233874397577248, 1.9287658245705757,
                          -1.452499762553149, 1.1231782793676652, 1.807815749861069]
        result = self.move_joints(joint_position)
        ...
        # Rik's commit
        # joint_position = [0.421765444596481, -1.2758873967804385, -0.3795983131679884, 1.9697008601575419,
        #                   -1.4848280204349968, 1.105926761423693, 1.5039123484760495]

        joint_position = [0.36005257892896025, -0.9906552257086317, -0.3387632029500069, 2.1292216382029174, 
                          -1.4527693634105807, 1.123133978502164, 1.8076478074873534]
    ...
    def tiago_move_to_side_position(self):
        joint_position = [0.1557893900857792, 1.0001510647387093, 1.1711949656216172, 1.592809800683023,
                          2.074395464296499, -1.4498072401510553, 0.660501527149556]
        return self.move_joints(joint_position)

    def alice_move_to_nav_position(self):
        joint_position = [2.002189804287326, 3.308477530858763, 0.9507020403543134, 6.109938651883328,
                          0.8985650192069583, 3.3495925735780903]
        return self.move_joints(joint_position)
